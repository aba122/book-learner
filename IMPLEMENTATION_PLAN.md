...
> 2026-09-08 实施状态:2.1–2.5 代码全部合入 main(PR #9–#17,执行计划 `docs/superpowers/plans/2026-09-08-m2-learning-system.md` T0–T8),core/web/src-tauri 三套测试与 CI 绿。**未完成**:2.3 中"profile.md 个人情境节的 AI 提取与确认流"未做(当前为用户在设置页手改;提取流留 M3);M2 验收的桌面目检(通知/托盘/番茄钟/三类书附加环节)见 `docs/smoke/m2-gate.md`,签字后在 main 打 `m2`。

## M3 体验完善 —— 「语音、导出、终评、打磨」

| # | 任务 | 说明 / 依赖 |
|---|---|---|
| 3.1 | 语音输入 | ✅ PR #29(M3 T3):whisper-rs 本机转写、模型**手动导入**(应用内下载不做)、录音→转写→填入输入框可编辑(§TECH 8) |
| 3.2 | Obsidian 导出 | ✅ PR #26(M3 T2):导出器(§TECH 9)、目标目录设置、增量重导 |
| 3.3 | 整书终评 | ✅ PR #25(M3 T1):全块通过后地图页入口、阶段化终评会话(§TECH 6.8)、学习报告 artifact + `_report.md` 归档 + 书标为已学完 |
| 3.4 | 阅读器打磨 | ✅ PR #28(M3 T4):高亮/书签/阅读位置、行高/缩进/出版方样式覆盖开关、学习模式块下划线(§TECH 7.1);霞鹜文楷内置与手动锚点校正未做 |
| 3.5 | 数据安全 | ✅ PR #27(M3 T5):`VACUUM INTO` 快照与保留策略、恢复登记、git 远程校验与退避推送;块删除本就进 skipped |
| 3.6 | 收尾 | M3 T6(feat/m3-t6-wrapup):文案巡检、抽取分批让出主线程 + 进度、阅读器首屏骨架、codex 路径设置项、OPF 书名/作者、薄弱点去重、`bundle.active` + dmg |
