...
describe('Tauri wire contract fixture', () => {
  it('固定命令名、顶层 payload key 与 unsupported capability', () => {
    ...
    expect(tauriWireContract.commands).toEqual([
      ...
      // Mac M2 T5:通过后附加环节
      { method: 'extraStart', command: 'extra_start', payloadKeys: ['blockId', 'kind', 'clientRequestId'] },
      ...
      // M3 T4:阅读器标记
      { method: 'readerMarkList', command: 'reader_mark_list', payloadKeys: ['bookId'] },
      { method: 'readerMarkAdd', command: 'reader_mark_add', payloadKeys: ['bookId', 'mark'] },
