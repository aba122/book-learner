 ...
 * 卡片高度按内容自适应:渲染后量 DOM 高度回传给布局。
 * 第二批:缩放(−/+/适配)与键盘导航(←→↑↓ 按阅读顺序选节点、Home/End、Esc 取消)。
 */
export default function LineageGraph({ graph, selectedId, onSelect, onDeselect }: Props) {
  ...
  const fit = () => {
    ...
    setZoom(clampZoom(Math.min((el.clientWidth - 8) / width, (el.clientHeight - 8) / height, 1)))
  }
  const scrollTo = (id: string) => {
    const el = Array.from(scrollRef.current?.querySelectorAll<HTMLElement>('[data-id]') ?? []).find(e => e.dataset.id === id)
  ...
  const onKeyDown = (e: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (e.key === 'ArrowRight' || e.key === 'ArrowDown') next = i < 0 ? 0 : Math.min(nodes.length - 1, i + 1)
