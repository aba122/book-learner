...
impl ai::AiProvider for MockProvider {
    fn complete(&self, _req: &ai::CompletionRequest) -> Result<String> {
        Ok(self.0.clone())
    }
}
...
fn full_block_lifecycle() {
    ...
    .unwrap();
    let blk1 =
        models::insert_block(&conn, book, "供给与需求", 1, "供需弹性", "elasticity", &[]).unwrap();
    models::insert_block(
        &conn,
        book,
        "供给与需求",
    ...
    assert_eq!(
        q0.iter().map(|t| t.kind.as_str()).collect::<Vec<_>>(),
        ["new", "new"]
    );
    ...
    let map_rows: Vec<(String, String)> = blocks
        .iter()
        .map(|b| (b.title.clone(), b.status.clone()))
        .collect();
    mem.sync_map("microecon", "微观经济学", &map_rows).unwrap();
    mem.commit(&format!("study: 微观经济学/供需弹性 {day0}"))
        .unwrap();
    ...
    assert_eq!(
        q1.iter().map(|t| t.kind.as_str()).collect::<Vec<_>>(),
    ...
    let wst: String = conn
        .query_row("SELECT status FROM weak_point WHERE id=?1", [wp_id], |r| {
            r.get(0)
        })
        .unwrap();
    assert_eq!(wst, "fixed");
