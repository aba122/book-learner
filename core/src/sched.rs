...
pub fn generate_daily(conn: &Connection, date: &str) -> Result<Vec<DailyTask>> {
    ...
    // 1) 薄弱点重考(按 created_at,上限 3)
    {
        ...
        let rows: Vec<(i64, i64, i64)> = st
            .query_map([WEAK_RETEST_DAILY_LIMIT], |r| {
                Ok((r.get(0)?, r.get(1)?, r.get(2)?))
            })?
            .collect::<rusqlite::Result<_>>()?;
        for (wid, block_id, book_id) in rows {
            insert(book_id, block_id, "weak_retest", 10, Some(wid))?;
        }
    }
    // 2) 到期间隔复习
    {
        ...
        let rows: Vec<(i64, i64, i64)> = st
            ...
            .collect::<rusqlite::Result<_>>()?;
        for (rid, block_id, book_id) in rows {
            insert(book_id, block_id, "review", 5, Some(rid))?;
        }
    }
    // 3) 新块(活跃计划配额)
    {
        let plan: Option<(i64, i64)> = tx
            .query_row(
...
pub fn on_block_passed(conn: &Connection, block_id: i64, date: &str) -> Result<()> {
    conn.execute(
        "UPDATE knowledge_block SET status='passed', passed_at=?2 \
...
pub fn on_review_result_with(
    ...
    if pass {
        conn.execute(
            "UPDATE review_schedule SET status='done' WHERE id=?1",
            [sched_id],
        )?;
        match stage {
            1 | 3 | 7 => {
                ...
                conn.execute(
                    "INSERT INTO review_schedule(block_id,stage,due_date) VALUES(?1,?2,?3)",
...
pub fn on_weak_retest(conn: &Connection, weak_id: i64, pass: bool, date: &str) -> Result<()> {
    if pass {
        conn.execute(
            "UPDATE weak_point SET pass_streak=pass_streak+1 WHERE id=?1",
            [weak_id],
        )?;
        conn.execute(
            "UPDATE weak_point SET status='fixed', fixed_at=?2 \
...
mod tests {
    fn setup() -> (rusqlite::Connection, i64) {
        ...
        let b =
            crate::models::insert_book(&conn, "书", "", crate::models::BookType::Textbook, "bk")
                .unwrap();
        for i in 1..=6 {
            crate::models::insert_block(&conn, b, "m", i, &format!("块{i}"), &format!("b{i}"), &[])
                .unwrap();
        }
        conn.execute(
    ...
    fn daily_queue_orders_weak_then_review_then_new() {
        ...
        for i in 0..4 {
            conn.execute(
                ...
                rusqlite::params![format!("wp{i}"), format!("2026-08-2{i}")],
            )
            .unwrap();
        }
        conn.execute(
            "INSERT INTO review_schedule(block_id,stage,due_date) VALUES(2,1,'2026-08-30')",
            [],
        )
        .unwrap();
    ...
    fn generate_daily_is_idempotent() {
        ...
        let n: i64 = conn
            .query_row(
                "SELECT count(*) FROM daily_task WHERE date='2026-08-30'",
                [],
                |r| r.get(0),
            )
            .unwrap();
    ...
    fn review_pass_advances_and_consolidates() {
        ...
        for (day, expect_next) in [("2026-08-31", 3i64), ("2026-09-03", 7), ("2026-09-10", 14)] {
            let id: i64 = conn
                .query_row(
                    "SELECT id FROM review_schedule WHERE block_id=1 AND status='due'",
                    [],
                    |r| r.get(0),
                )
                .unwrap();
            ...
            let next: i64 = conn
                .query_row(
                    "SELECT stage FROM review_schedule WHERE block_id=1 AND status='due'",
                    [],
                    |r| r.get(0),
                )
                .unwrap();
        ...
        let due_left: i64 = conn
            .query_row(
                "SELECT count(*) FROM review_schedule WHERE block_id=1 AND status='due'",
                [],
                |r| r.get(0),
            )
            .unwrap();
    ...
    fn review_fail_resets_and_creates_weakpoint() {
        ...
        super::on_block_passed(&conn, 1, "2026-08-30").unwrap();
        let id: i64 = conn
            .query_row("SELECT id FROM review_schedule WHERE block_id=1", [], |r| {
                r.get(0)
            })
            .unwrap();
    ...
    fn apply_eval_to_db_persists_status_scores_weakpoints_and_schedules() {
        ...
        let e: crate::eval::EvalResult = serde_json::from_str(
            ...
            "summary":"s","final_restatement":"r","weak_points":[
              {"title":"未修复点","detail":"d"},
              {"title":"已当场修复","detail":"d","fixed_in_session":true}]}"#,
        )
        .unwrap();
        super::apply_eval_to_db(&conn, 1, &e, "2026-08-30").unwrap();
        let (st, passed_at, scores): (String, Option<String>, Option<String>) = conn
            .query_row(
        ...
        let n2: i64 = conn
            .query_row(
                "SELECT count(*) FROM review_schedule WHERE block_id=1 AND stage=1",
                [],
                |r| r.get(0),
            )
            .unwrap();
    ...
    fn weak_retest_two_passes_fixes() {
        let (conn, _) = setup();
        conn.execute(
            "INSERT INTO weak_point(block_id,title,created_at) VALUES(1,'w','2026-08-29')",
            [],
        )
        .unwrap();
        ...
        let st: String = conn
            .query_row("SELECT status FROM weak_point WHERE id=1", [], |r| r.get(0))
            .unwrap();
        assert_eq!(st, "fixed");
    }
    #[test]
    ...
    fn behind_two_days_triggers_replan() {
        ...
        let updated: i64 = conn
            .query_row(
                "SELECT daily_new_blocks FROM study_plan WHERE book_id=?1",
                [b],
                |r| r.get(0),
            )
            .unwrap();
