...
| 命令 | payloadKeys | 返回 |
|---|---|---|
| `reading_topics` | `bookId` | `ReadingTopic[]`(id、startedAt、endedAt、distilled、anchorHref、firstQuestion 前 20 字) |
| `reading_messages` | `topicId` | `ReadingMessage[]` |
...
| 场景 | 处理 |
|---|---|
| codex 超时/失败 | 用户消息 failed,气泡上「重试」;错误文案沿用 AsyncError |
...
`READING_CHAPTER_MAX_CHARS=6000`、`READING_QUOTE_WINDOW=3000`、`READING_HISTORY_TURNS=8`、`READING_STATE_MAX=40`、`READING_NOTES_PER_BLOCK=10`,放 `core` 常量并在 `web/src/config.ts` 镜像展示用值。
