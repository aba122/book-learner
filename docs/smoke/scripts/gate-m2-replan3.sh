...
sql "update knowledge_block set skipped=0 where book_id=$ACTIVE;"
sql "update study_plan set deadline='$(date -v+23d +%F)' where book_id=$ACTIVE;"
SK=$(sql "select count(*) from knowledge_block where book_id=$ACTIVE and skipped=1")
echo "== 顺延分支:推进到 $(date -v+25d +%F)(截止 $(date -v+23d +%F) 已过,跳过块=$SK)"
a set bookLearner.testDate $(date -v+25d +%F) >/dev/null; a js "location.reload(); return 1" >/dev/null; sleep 6; front
...
echo "GATE-M2-REPLAN3-DONE $(date)"
