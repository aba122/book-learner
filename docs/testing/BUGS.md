...
| BL-011 | P2 | fixed | 阅读器 | 用户口述(09-11 14:0x):"改成这版后,鼠标要点到页面最左边和最右边才能翻页,之前鼠标点左右半部分就能翻页" | PR #40 的翻页点击区只有两侧各 48 px 透明条 | PR #43 `clickToTurn.ts`:监听挂在正文 iframe 的 document 上,没拖动(≤8 px)、没划选、按下时无选区、非链接、无修饰键的单击才翻;分页 iframe 比可视区宽,用 frameElement 位置换算到父文档再比可视区中线;两侧边条保留 | clickToTurn.test 4、reader.test「BL-011」 | 09-11(用户) |
