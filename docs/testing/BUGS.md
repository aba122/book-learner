...
| BL-010 | P3 | open(已退回) | 阅读器 | 用户口述(09-09 19:0x):"翻页可以加上翻书的那种显示特效,而不是干巴巴的翻页";换包 21c2803 后口述(09-11 11:2x):"我要的是纸质书翻页的那种特效";换包 6dc0831 后口述(09-11 13:5x):"翻书功能做得不好,退回上个版本先吧";追问后口述(09-11 14:0x):"翻页的过程太粗糙了,整体就不像翻书" | 无过渡;滑入(PR #40)不算;3D 两拍翻页(PR #41)观感不好 | PR #42 退回 PR #40 的滑入过渡(保留 BL-002/003)。用户的标准是"像翻书":整页 3D 硬翻不算,要的是纸质卷页(页角卷起、纸面弯曲、背面透字、单侧翻)。做法只剩页面快照(克隆当前页 iframe 文档到卷页层)+ 卷页着色器/曲面变形,大章(CFA 百 k 字)快照代价要先测;下次开工前先做一个静态 demo 给用户过目再接进阅读器 | reader.test「BL-010」「BL-009」 | 09-09(用户);09-11 返工;09-11 退回 |
| BL-011 | P2 | open | 阅读器 | 用户口述(09-11 14:0x):"改成这版后,鼠标要点到页面最左边和最右边才能翻页,之前鼠标点左右半部分就能翻页" | PR #40 的翻页点击区只有两侧各 48 px 透明条(`page-zone-prev/next`),正文区域没有点击翻页 | 待做:点左半页 prev、右半页 next(单击且无拖选才翻,不能挡住鼠标划选高亮)。首选在 epub.js `rendered` 时给正文 iframe document 挂 click 监听(划选后 `getSelection()` 非空则不翻),保留两侧窄条兜底;须在 Mac 上用真实鼠标事件(System Events click at)验证 | 待补 reader.test | 09-11(用户) |
