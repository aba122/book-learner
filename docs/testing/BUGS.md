...
| BL-012 | P1 | open | 阅读器 | 用户口述(09-17):"书里的文字不能 copy" | 疑似 BL-011 的父文档指针层 `.bl-pointer`(z-index 5、`user-select:none`)盖在正文上,拦截 mousedown/mouseup 并禁用选择,原生复制被挡;选区靠轮询造出来但无法触发系统复制菜单 | 待定:指针层允许划选后不吞 copy(如按下即透传给 iframe、或选区存在时让出 pointer-events),或提供「复制选区」按钮 | 待补 | 09-17(用户) |
| BL-013 | P2 | open | 阅读器 | 用户口述(09-17):"打开夜读模式时,书页正文没有跟着变,只有边框变成夜读模式" | 侧栏「夜读模式」切的是 app 的 `data-theme=dark`(只管外壳 chrome);阅读器正文 iframe 用的是独立的阅读主题(纸白/羊皮/夜读,存 readerPrefs.theme),两者不联动 | 待定:app 夜读模式联动阅读器主题(切 dark 时正文用 night 主题),或在正文区跟随 data-theme | 待补 | 09-17(用户) |
