...
| BL-008 | P3 | fixed | 阅读器 | 用户口述(09-09 19:0x):"阅读界面可以设置成双页" | 阅读设置固定 `spread: 'none'` | 阅读设置加「双页显示」(prefs.spread → `rendition.spread('auto'/'none')`) | reader.test「BL-008」 | 09-09(用户) |
...
| BL-010 | P3 | fixed | 阅读器 | 用户口述(09-09 19:0x):"翻页可以加上翻书的那种显示特效,而不是干巴巴的翻页" | 无过渡 | 翻页时新页从翻页方向滑入(220 ms,`data-turning` + CSS 动画;`prefers-reduced-motion` 关闭);真实卷页特效不做 | reader.test「BL-010」 | 09-09(用户) |
