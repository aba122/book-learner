...
| BL-010 | P3 | open(返工) | 阅读器 | 用户口述(09-09 19:0x):"翻页可以加上翻书的那种显示特效,而不是干巴巴的翻页";换包 21c2803 后用户口述(09-11 11:2x):"我要的是纸质书翻页的那种特效"——滑入过渡不算,要纸质书卷页/翻角效果 | 无过渡;PR #40 只做了滑入 | PR #40 滑入(220 ms,`data-turning` + CSS 动画)已合入但不满足;待做:仿纸质书翻页(3D 卷页或翻角动画,单页/双页都要成立,`prefers-reduced-motion` 关闭) | reader.test「BL-010」(待补卷页用例) | 09-09(用户);09-11 返工 |
