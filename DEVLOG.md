...
## 2026-09-11 · BL-010 退回(PR #42)
- 用户在 6dc0831 上看过两拍 3D 翻页后说"翻书功能做得不好,退回上个版本先吧"。`git revert d543791` 退回 PR #40 的滑入过渡(`READER_PAGE_TURN_MS=220`),BL-002/BL-003 保留。台账 BL-010 改回 open(已退回),下次动手前先问清不满意的具体点;真正的纸质卷页(单侧、正文随页弯曲、背面透字)需要页面快照,另行评估。
...
## 2026-09-20 · 视觉改版第一批(基础层:令牌 v2 / 图标 / 原语 / 外观三态 / 窗口 chrome / 菜单栏)
- 起因:用户要求整体优化美观与设计,参考 developer.apple.com/design 与 `dickwu/apple-design-skill`。先装 skill(`~/.claude/skills/apple-design`,HIG 122 页),用它出了基线审查 `docs/design/2026-09-20-hig-review-baseline.md`(总评 Critical issues:三级字/复习色对比度不足、对话框无键盘出口、焦点环被 `outline-none` 抹掉;High:默认窗口 chrome、app 级外观开关、任务色当强调色、高亮色硬编码、设置页不合惯例)。计划见 `~/.claude/plans/mac-app-epub-lucky-spring.md`,四个方向已与用户确认:纸感为骨原生为皮、透明标题栏 + 侧栏毛玻璃、SF 风格自绘 SVG 图标、分三批。
- **令牌 v2**(`tokens.css` 重写 + `index.css` 映射):原语 → 语义两层(`surface-*`/`label-1..4`/`separator`/`fill-*`/`accent`/`signal-*`/`hl-*`),旧名 `--ink-*`/`--paper-*`/`--line`/`--c-*`/`--shadow-*` 保留为别名(第三批末删)。对比度实算:亮色三级字 #857a68(3.7)→ #6b6150(5.4),复习色 #a8741f(3.6)→ #8c5f12(5.0),ok/warn 也调深;强调色 `--accent` #3b5aa6(5.8)且 `@supports (color: AccentColor)` 时跟随系统强调色;暗色三级 #9a8f77(5.7)、强调 #9db3ea(8.7)。**四级字只准装饰**,`tokensPolicy.test` 棘轮(可点元素 + label-4 同行 ≤17,第二批 ≤8,第三批 0)+ 禁 Tailwind 调色板色。字阶按 macOS 文字样式(`text-large-title 26/32 … text-footnote 10/13`);`--font-sans` 以 `-apple-system, BlinkMacSystemFont, "SF Pro Text", "PingFang SC"` 起头;补 `--font-reading`;圆角 4/6/8/12/16、三档层级阴影、动效 120/200/320 ms、z 轴。**删纸纹**与两个死令牌。全局 `prefers-reduced-motion` 守卫(`data-motion-essential` 例外)、`prefers-contrast: more`、`prefers-reduced-transparency` 回退;**不写 `::-webkit-scrollbar`**(WebKit 会退回常显经典滚动条)。高亮四色令牌化(`readerThemes.highlightFill()` 读 `--hl-*`/`--hl-alpha` 出 `color-mix`;`EpubView` 描边读 `--reader-block-underline`);`prefersReducedMotion` 搬到 `lib/motion.ts` 并加 `useReducedMotion()`。
- **图标**:`components/icons/paths.ts`(52 个自绘 SF 风格线性图标,24 网格、活区 20、1.5px 描边、`vector-effect: non-scaling-stroke`)+ `Icon`(无 label 即 `aria-hidden`,有则 `role=img`)。SF Symbols 只授权 Apple 平台,故自绘。
- **原语**(`components/`):`Dialog`(portal、`aria-modal`、`useFocusTrap` Tab 循环、Esc、焦点还原、`modalStack` 在 `<html>` 打 `data-modal-open`;默认焦点落面板本身让读屏先读标题)、`Confirm` 重写为 Dialog 薄壳(danger 默认焦点在取消)、`Field`(`useId`,hint/error 经 `aria-describedby`,error 带 `role=alert`)、`Input/Textarea/Select/Checkbox/Toggle`(共用 `lib/formClasses`,焦点 3px 强调环)、`Segmented`(radio|tabs 语义、逐项 `ariaLabel`、roving tabindex,选中段凸起白段不再靛色填色)、`Toolbar`(52px 带、`data-tauri-drag-region`,侧栏折叠时自带「显示侧栏」)、`Tooltip`(render-prop,无 ref、无第三方库;悬停 400 ms / 聚焦即显;`aria-describedby`)、`IconButton`(28×28,`aria-label` + sr-only 文字——自动化桥按 innerText 点按钮)、`EmptyState`、`Skeleton`、`Spinner`;`Button` 改 28px/圆角 6,档位 primary/secondary(带边框,默认)/ghost(无边框)/danger + size;`PageHeader` 用 `text-large-title`。`AsyncError`、`Tag`、`Card` 未动(类名被测试钉住/别名生效)。
- **外观跟随系统**(`store.ts`):`themePreference: system|light|dark`,`theme` 仍是解析值(BL-013 耦合与旧测试不改);`matchMedia` 守卫(jsdom 无);跟随系统时系统切换即刻生效;`setTheme('system')` 删键。**删侧栏底部夜读按钮**,设置页通用组首行「外观」三态 `Segmented`(即时生效,不进保存快照)。侧栏折叠状态 `SIDEBAR_KEY` 持久化。
- **外壳**(`App.tsx`):侧栏 `w-60 bg-sidebar`,顶部 52px 拖动带(红绿灯落这里),右上「隐藏侧栏」,`<nav aria-label="主导航">` 图标 + 文字行(`NavLink` 自带 `aria-current`,选中中性填充 + 强调色图标,去掉赤色刻线),底部无操作;主区 `bg-content` + 第一批过渡用的 52px 拖动带 `MainTitleBand`(折叠时给「显示侧栏」;第二批起各页 `Toolbar` 取代)。`MenuActions` 订阅 `backend.subscribeMenu`(`open-settings` → 设置页,`toggle-sidebar`);`tauri.ts` 监听 `menu_action` 事件(不在 wire contract 里,同 `pomodoro_changed`),`mock.ts` 用 ⌘, / ⌃⌘S 键盘映射走同一契约。阅读器全局 ←/→ 在 `isModalOpen()` 时不翻页。设置页与语音区的 `inputCls/areaCls` 全部换成原语(修掉 `outline-none`)。
- **窗口 chrome**(`tauri.conf.json`):`macOSPrivateApi: true`,窗口 `titleBarStyle: Overlay`、`hiddenTitle`、`trafficLightPosition {14,20}`、`transparent`、`windowEffects: sidebar / followsWindowActiveState`——`window-vibrancy 0.6` 已是 tauri 2.11 传递依赖,纯配置零 Rust;`capabilities/default.json` 加 `core:window:allow-start-dragging`(`core:default` 不含,否则拖动区静默失效)。`main.tsx` 打 `data-shell=tauri|web`,Tauri 下 html/body 透明让材质透出,浏览器 mock 铺纸色。
- **菜单栏**(`lib.rs::install_app_menu`,`#[cfg(target_os = "macos")]`):攻书(关于/设置… ⌘,/服务/隐藏/退出)、编辑(撤销…全选——替换默认菜单后必须自建,否则 WebView 里 ⌘C/⌘V 失效)、显示(隐藏/显示侧栏 ⌃⌘S、全屏)、窗口(最小化/缩放/关闭窗口 ⌘W,与 CloseRequested→hide 一致)、帮助(打开日志文件夹 → `diagnostics::reveal_logs`);动作 `app.emit("menu_action", …)`。favicon 换纸底「攻」字;标题「攻书」。
- **Craft 取舍**:保留宋体字标/标题 + 三任务色只标数据;删纸纹、赤色刻线、靛色 tab 填色、夜读按钮;Toast 留第二批(常驻 `role=status` 会撞 `today.test.tsx:700`)。
- 门禁:web 415/2(+Icon 3、原语 10、store +4、App +3、settings +1、tokensPolicy 2)、tsc、oxlint 0、build;壳层 rustfmt;clippy/foundation/mac-foundation 待 CI;Mac 实测清单见计划。
- **Mac 实测(调试构建,真实数据只读副本)**:`data-shell=tauri`、html/body 透明、`aria-current`、菜单栏「Apple, book-learner, 编辑, 显示, 窗口, 帮助」;⌃⌘S 折叠/展开侧栏、⌘, 进设置、Tab 焦点环 2px 可见、系统外观切换 `data-theme` 即刻翻转(切完已还原)、书架「删除」确认框 Esc 关闭且默认焦点在「取消」、`data-modal-open` 正确收放。**AccentColor 实验撤回**:WKWebView 里 `AccentColor` 关键字只对 `color/fill` 解析成系统蓝,`background-color` 变透明、`border/outline-color` 变 currentColor——`bg-accent` 的主图标钮/开关/复选框会消失,故固定用 #3b5aa6 / #9db3ea。红绿灯:AX 报告按钮顶在带内 y=10(配置 20),改为 30 使其在 52px 带内垂直居中(release 装机后再目视确认)。App 菜单标题显示进程名 book-learner 而非「攻书」(由 CFBundleName 决定,改 productName 会改 .app 名与安装脚本路径,暂不动,Low)。截图回传两条路(`screencapture`、模拟 ⇧⌘3)都因屏幕录制权限/快捷键落空,毛玻璃与整体视觉留待「换」后目视。

## 2026-09-20 · 视觉改版第二批(今日 / 书架 / 阅读器 + Menu/Popover/ProgressBar/Toast)
- **共享准备**:`Menu`(受控,`role=menu` + 28px `menuitem`,按钮锚定或右键坐标,↑↓ Home End Enter Esc、外点关闭、焦点回锚定钮;`MenuEntry` 联合类型用 `isSeparator` 守卫收窄)、`Popover`(非模态锚定面板 `role=dialog aria-modal=false`,焦点陷阱、Esc/外点关闭、焦点回锚)、`ProgressBar`(定/不定,`size sm|md`)、`ToastHost` + `lib/toastStore`(每条自带 `role=status`,无条目时不渲染任何 DOM——避免撞 `today.test.tsx:700`/`settings.test` 的单一 status 查询)。`features/reader/markdown` 下沉为 `components/Markdown` + `lib/markdownParse`(费曼/终评第三批复用)。`Dialog` 加 `label`(可访问名固定,标题随阶段变化的向导用)与 `closeButton`(页脚已有取消/关闭时关掉头部 ×,否则「关闭」重名)。`IconButton` 改 `forwardRef`(Popover/Menu 锚定)并加 `expanded`(aria-expanded,外观同 active);`Textarea` 转发 ref、给了 `rows` 不再强加 `min-h-20`。
- **书架**:工具栏带右侧「+ 导入书籍」primary sm;栅格 `repeat(auto-fill, minmax(10.5rem, 1fr))`(按容器,不按视口——侧栏占宽后视口断点会错);封面保留“首字 + 书脊色条”签名,主攻书 `ring-2 ring-accent`;瓦片 `aria-label="打开《x》"`;四个下划线小字动作 → 「阅读」secondary sm + `IconButton ellipsis-circle`「《x》的更多操作」打开 `Menu`(导出到 Obsidian / 标记为已学完 / — / 删除〔红,`ariaLabel="删除《x》"`〕),卡片右键同菜单;空态 `EmptyState`(标题原文)+「导入 EPUB」;骨架 4 瓦;删除成功 toast「已删除《x》」。导入向导 → `Dialog lg`(`label="导入书籍"`,忙态不可关,`ProgressBar` 不定 + 进度文案原文,拖放区 `tabIndex=0` 可键盘触发);导出 → `Dialog xl`(页脚 取消/确认导出 → 在 Finder 中显示/完成)。测试:`library.test` 6 处改为“先点更多操作再点 menuitem”;门禁脚本 `gate-m2-dev.sh`/`gate-m3.sh` 同步。
- **今日**:工具栏带右侧**番茄钟胶囊**(`data-testid="pomodoro"` 保留:timer 图标 + `tabular-nums` 时间独立元素 + 阶段·任务名〔`@lg` 以下隐藏任务名,Toolbar 加 `@container`〕+ ghost sm「暂停/继续」「结束」;控制失败 → 胶囊尾部警示钮自动展开 `Popover` 挂 `AsyncError compact`,关掉后可再点开——用“已看过的错误对象”派生 open,不在 effect 里 setState);右下浮动卡删除。页头右侧统计簇保留为数据(`ProgressRing 56`)。`TaskCard`:左 4px 色条(签名)+ 序号 01/02…(队列是真序列:薄弱重考 → 复习 → 新块)+ 衬线 `text-title3` 标题 + 预计时长升到二级色 + 按钮 sm(一 primary 其余 secondary,窄容器换行)。跨页提示与自动均摊提示合进**同一个** `role=status`;加载 3 行骨架;空态 `EmptyState`(标题原文)+「去书架」;`ReplanDialog` → `Dialog lg`(Esc/遮罩 = 本日不再提醒,忙态不可关)。`App.tsx` 过渡带 `MainTitleBand` 只在未换页(地图/费曼/终评/统计/设置)渲染,第三批换完删。
- **阅读器**:`Toolbar aria-label="阅读器工具栏"`:返回(`chevron-left`;回读原文时仍是文字「返回讲授」)· 衬线标题 · 目录/书签/标记/阅读设置(`IconButton`,目录与阅读设置 `aria-haspopup="dialog" aria-expanded`)· 分隔 · 放大/收窄(对话)· 收起侧栏/展开侧栏(标签随状态变,不叠 aria-pressed)。目录 → `Popover`(28px 行);阅读设置 → `Popover`(A−/A+ 文字钮名字原文、主题三色块 `aria-pressed`、行高仍是 `aria-pressed` 按钮组〔测试按 `button` 角色查〕、三个 `Checkbox`);右栏改平铺检视列(`border-l bg-content`,不再 `m-3` 浮动 Card),三 tab → `Segmented tabs`(图标 book-closed / quote-bubble / map,去掉 💬🗺,可访问名不变,`aria-controls` 指向 `role=tabpanel` 的面板,`hidden` 语义与 testid 不变〔BL-017〕);翻页 → 36px 圆钮悬停/聚焦显现;进度条 → `ProgressBar sm` 用 accent(不再用复习琥珀);选区/高亮工具条改 `bg-popover` 浮条 + `bg-hl-*` 色块 + ghost 动作;`MarksPanel` 34px 行 + `IconButton trash/xmark`(删除钮 aria-label 原文;「收起 ›」→「关闭」);问书:`Select sm` 历史话题、气泡 `text-body`、行内重试/刷新/取消为 accent 文字钮、`Spinner` 思考中、`Textarea`;脉络图:头部「更新到最新进度」primary sm /「重新生成」secondary sm(原 ink-4),确认框 secondary/danger,`EmptyState icon=map`,详情浮层 `Input/Textarea`(修 3 处 `outline-none`),看原文 secondary sm、问一问 ghost sm + 图标,修正折叠区 `aria-expanded` + `Textarea`,缩放 `IconButton`(名字原文),选中节点 `border-accent`。全局 ←/→ 翻页:焦点在 `[role=dialog]/[role=menu]` 内也不翻(浮层非模态,`isModalOpen()` 管不到)。
- 文案/测试改动:`readingChat.test:238`「收起 ›」→「收起侧栏」;`reader.test:259/332` `queryByRole('toolbar')` 改按名「选区操作」(带本身现在是 toolbar)。新增:书架(工具栏含导入、右键菜单、空态动作、删除 toast)、今日(胶囊在工具栏内、序号、空态导航、Esc 关重排)、阅读器(工具栏钮与 `aria-controls`、阅读设置 Esc 回焦 + 浮层开着 → 不翻页、目录浮层跳转)。`tokensPolicy` 阈值 17 → 2(剩两处在费曼页,第三批收 0)。
- 门禁:web 422/2、tsc、oxlint 0、build;PR #59 → main `a1df5a4`。
- **Mac 实测(调试构建 `target/debug/book-learner`,真实数据只读副本,DOM/AX 检查)**:今日工具栏含番茄钟胶囊(`24:58 | 专注中 · 块名 | 暂停 | 结束`,带 1040/1040 无溢出),暂停/结束经后端生效;书架 1280 下 4 列、960 下 3 列,「更多操作」菜单三项、打开即焦点进 menuitem、Esc 关闭后焦点回「《x》的更多操作」、右键卡片出菜单;阅读器工具栏 返回/目录/书签/标记/阅读设置/放大对话/收起侧栏,tab 的 `aria-controls` 指向两个 `tabpanel`,阅读设置浮层 288×257 锚在钮下右对齐、`aria-expanded=true`、Esc 关闭(脚本用 JS `.click()` 打开,钮没拿到焦点,所以焦点回 body——WebKit 鼠标点按钮同样不聚焦;键盘路径由单测覆盖),目录 7 项,标记面板「书签与高亮」,收起侧栏后重开条在、展开/放大 640px,进度条 35%,翻页圆钮 36px 悬停显现;深色跟随系统即刻生效(`data-theme=dark`,右栏底 rgb(25,21,15))并已还原;960×640 下今日页无横向溢出、阅读器带 720/720。主区不再有过渡带(`mainband=0`)。
- 教训:自动化桥 `js` 曾连续约 20 分钟对所有构建(含第一批提交)超时,而 `run.log` 里 `command="planning_today_queue"` 等仍在正常打印——前端活着,是 Mac 侧临时状态(不是代码);之后同一二进制、同一启动方式全部通过。以后先看 `command=` 日志再决定要不要二分。

## 2026-09-20 · 视觉改版第三批(设置 / 知识地图 / 费曼·终评 / 统计;删旧名别名)
- **设置**(`SettingsPage.tsx`、`VoiceSection.tsx`、新 `SettingsSection.tsx` + `settingsSections.ts`):工具栏带右侧「已保存 + 保存」;内容区左列 144px 粘性分区导航(通用 / AI 与导出 / 语音 / 画像 / 数据 / 诊断,`nav aria-label="设置分区"`,当前项 `aria-current`,点击 `scrollIntoView`,滚动跟随用 `IntersectionObserver`,jsdom 无则只响应点击;容器窄于 `@3xl` 时隐藏),右侧**六个分区全部渲染不隐藏**(真隐藏会让 12 条 `getByRole` 测试找不到元素)。每个分区 = 衬线 `text-title3` 标题 + 说明 → `Card p-0 divide-y` 分组内嵌列表,行 44px(`Field row` / `SettingsRow`);语音区模型行 44px、原生 radio(`accent-accent`)、删除钮 `IconButton trash`(aria-label 原文);画像区标题右侧「画像已保存 + 保存画像」;数据区快照行 34px;诊断 `dl` 行 34px。`Field` 加 `id` 固定项——门禁脚本 `gate-m3.sh` 按 `input[id="field-codex 可执行路径"]` 找 codex 输入框,这个 id 自第一批换 `useId` 起就已失效,现改 `input#codex-bin`。
- **知识地图**(`MapPage.tsx`):带上 浏览态「整书终评」(全通过时 primary)+「编辑地图」,编辑态「取消」+「确认定稿」;每模块一张 `Card p-0 divide-y`(模块名行 + 44px 块行:序号 01/02 衬线三级色、衬线标题、依赖注脚、Tag、`Stars role=img`);编辑态六个小文字钮 → 行内 `IconButton` 组(arrow-up/down 上移下移、eye-slash/eye 跳过恢复、arrow-merge 并入上一块、scissors 拆分/取消拆分〔`active`〕、trash 删除;名字原文;始终在 DOM,悬停/`focus-within` 显现),编辑态顶部一行提示替代原来 3 处长 `title=`;目标设定 → `Dialog sm`(`label="目标设定"`,`Field` 完成期限/提醒时间,Esc = 稍后再定)。
- **费曼**(`FeynmanPage.tsx`、`Transcript.tsx`、`EvalCard.tsx`、`ExtraStage.tsx`、`VoiceInput.tsx`):`Toolbar aria-label="讲授工具栏"`:`IconButton sidebar-left`「原文参考」(`aria-pressed`,开关状态存 `FEYNMAN_SOURCE_KEY`)· Tag · 唯一 h1「讲授/复习:x」· `SESSION_HINT`(`@xl` 以下隐藏,独立元素)· 回读原文 / 结束讲授(`data-ready`,就绪 primary)/ 放弃本次(sm)。原文参考 `aside aria-label="原文参考"`(`bg-inset/50`,正文 `font-reading`);气泡:用户反白墨色(签名)、学生卡面(`USER_BUBBLE/STUDENT_BUBBLE` 常量供三处复用);思考行 `Spinner` + 「学生思考中…」(桥 `wait/waitgone` 文案不变);打字机保留,`useReducedMotion()` 时首拍直接出全文;欢迎提示 `EmptyState compact`。`VoiceInput`:空闲 `IconButton mic`「语音输入」、转写中禁用胶囊 + `Spinner`、录音中红点胶囊(`aria-label="停止录音"`,电平 `voice-level`)+ ghost「取消」。`EvalCard` → `Dialog lg dismissible=false`(两个判定按钮就是出口;评分内嵌列表 `eval-stars role=img`、薄弱点列表、建议 callout、错误在卡内)。`ExtraStage` Shell → `Dialog xl`(`label="附加环节"`,`dismissible=false`,各阶段页脚自带出口;桥按 `[aria-label="附加环节"] h2` 读标题仍成立),结果 `<pre>` → `<Markdown>`。
- **终评**(`FinalExamPage.tsx`):会话态同带式(Tag · h1 · 提示 · 生成学习报告 / 回到地图);报告态带上「返回书架」primary,`PageHeader`「学习报告:x」+ `<Markdown>`(二级标题渲成段落,页面只有一个 h1),`final-overall` 与 `_report.md` 注脚保留。
- **统计**(`StatsPage.tsx`):空 `Toolbar`(只作拖动区);瓦片 `grid-cols-2 @3xl:grid-cols-4`(hero 跨两列两行,`ProgressRing 96`),数值 `text-large-title` 衬线 + 单位 `text-callout`;三张详情卡内部 `@2xl:grid-cols-3`;三个手绘图表各加 `aria-describedby` → `<table class="sr-only">`(caption:近 14 天每日投入数据表 / 打卡日历数据表 / 薄弱点趋势数据表),薄弱趋势图下加两色图例;书进度条加 `role=progressbar`;系列色不变。
- **外壳**:`App.tsx` 过渡带 `MainTitleBand` 与 `OWN_TOOLBAR` 删除——八个页面都自带 `Toolbar`;`<main>` 改 `overflow-hidden`(滚动区在各页内)。
- **令牌收口**:`tokensPolicy` 阈值 2 → **0**;删掉 `--ink-*/--paper-*/--line/--c-*/--shadow-*` 变量与 `--color-ink-*/paper-*/line`、`--shadow-pop` 映射,全站(Markdown/Tag/ProgressRing/AsyncError/Card/EpubView)改语义名;`AsyncError.test` 钉住的类名同步改为 `border-sep bg-card`。
- 测试:现有断言不动(设置 12 条 `getByRole`、地图 `block-item`/角色/`toBeDisabled`、费曼唯一 h1 与 dialog 名、终评 `final-overall`、统计 testid 全部保住);新增 设置 2(分区导航、分组列表)、地图 2(工具栏与行内钮、目标 Esc)、费曼 2(工具栏与原文参考开关、评估卡不可关)、终评 1(Markdown 报告)、统计 1(读屏表)。门禁:web 430/2、tsc、oxlint 0、build。
- 复审:`docs/design/2026-09-20-hig-review-after-batch3.md`(Critical 0,High 0)。
- **验证(headless Chromium 跑构建产物 + mock 后端,1280×800 / 960×640 / 深色)**:设置——工具栏带、左列六项、六个 h2 顺序、点「画像/诊断/通用/AI 与导出」滚到分区且高亮正确、回到顶部点亮「通用」、960 下左列隐藏且无横向溢出;地图——12 行 44px、编辑态行内 6 个图标钮默认 opacity 0、Tab 聚焦后 1、目标设定对话框焦点在框内、Esc 关闭;费曼——工具栏 原文参考/回读原文/结束讲授/放弃本次、唯一 h1、原文参考开关写 `bookLearner.feynmanSource=closed`、放弃确认框默认焦点「继续讲」;统计——1280 四列 / 960 两列、三个图表都指向 `<table>`、大数 26px;深色 `data-theme=dark` 卡面 rgb(33,28,20);无控制台错误。首次实现的 IntersectionObserver 跟随在点击后会被平滑滚动途中的回调抢走高亮(点「画像」停在「语音」),改为每次回调按当前几何取"最后一个过 1/4 线的分区"、点击后 1 s 内忽略回调、滚到底点亮最后一个。
- **Mac 真机**:第三批没有壳层改动,真机目视留到装机后。当天下午用户在用机(WindowServer 62%、微信/Chrome 活跃)时,调试构建的 WebView 100 s 内没有加载页面(后端零命令),自动化桥全部超时;与上午"约 20 分钟后自愈"是同一现象,推测与用户当前 Space/窗口遮挡有关,不是代码问题。以后跑真机实测先看 `uptime`/WindowServer,用户在用机就改 headless。

## 2026-09-21 · 装机后首轮反馈(BL-024:侧栏太蒙 / 日夜读入口 / 提示框出界)
- 用户装上三批改版(`d467280`)后反馈四条,前三条本节修,第四条(阅读时长统计)另开 BL-025。
